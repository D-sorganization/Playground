...
class SolarSystemScene:
    def __init__(self, settings: RenderSettings | None = None):
        ...
        # self.time_nav_panel is now visually inside UnifiedControlPanel but kept for
        # logic
    ...
    def _render_overlays(self, julian_date: float):
        ...
        if self.sidebar_panel:
            ...
            # Pass to sidebar renderer (which handles the sidebar frame + invokes
            # content)
    ...
    def _should_render_body(self, body: CelestialBody) -> bool:
        ...
        # For Moons, we might need a specific flag if we want to toggle them separately
            ...
            # For now, let's tie moon visibility to general minor bodies or parent
            # visibility?
            # User asked for granular. Let's start with minor bodies flag for
            # moons/asteroids.
