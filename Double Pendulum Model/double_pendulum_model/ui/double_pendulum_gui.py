...
class DoublePendulumApp:
    ...
    def _build_ui(self) -> None:
        ...
        row += 1
        labeled_row("Upper COM ratio", "0.45", row,
        ...
        row += 1
        labeled_row("Plane incline (deg)", str(DEFAULT_PLANE_INCLINATION_DEG), row,
        ...
        labeled_row("Shoulder damping", "0.4", row,
                   "Damping coefficient for upper segment (N·m·s/rad)")
        row += 1
        labeled_row("Wrist damping", "0.25", row,
        ...
        labeled_row("Shoulder torque f(t)", "0.0", row,
        ...
        start_btn = tk.Button(button_frame, text="Start", command=self.start, 
                             width=8, bg='#4CAF50', fg='white', font=("Arial", 10, "bold"))
        start_btn.pack(side=tk.LEFT, padx=2)
        
        pause_btn = tk.Button(button_frame, text="Pause", command=self.pause,
                             width=8, bg='#FF9800', fg='white', font=("Arial", 10, "bold"))
        pause_btn.pack(side=tk.LEFT, padx=2)
        
        reset_btn = tk.Button(button_frame, text="Reset", command=self.reset,
                             width=8, bg='#2196F3', fg='white', font=("Arial", 10, "bold"))
        ...
        tk.Label(granularity_frame, text="Granularity (every N steps):", bg='white').pack(side=tk.LEFT, padx=(20, 5))
    ...
    def _start_data_logging(self) -> None:
        ...
        self.data_file.writerow([
            'time', 'theta1', 'theta2', 'phi', 'omega1', 'omega2', 'omega_phi',
            'tau1', 'tau2', 'grav1', 'grav2', 'damp1', 'damp2',
            'coriolis1', 'coriolis2'
        ])
        self.data_step_counter = 0
    ...
    def _update_pendulum_immediately(self) -> None:
        ...
        try:
            ...
            # Update state with new angles, preserve velocities if running
            if self.state is None or not self.running:
                ...
                # Preserve velocities but update angles
                self.state = DoublePendulumState(
                    theta1=math.radians(user_inputs.shoulder_angle_deg),
                    theta2=math.radians(user_inputs.wrist_angle_deg),
            ...
            if self.state is None or self.dynamics is None:
                ...
                try:
                    from double_pendulum_model.physics.double_pendulum import (
                        DoublePendulumParameters, DoublePendulumState
                    )
    ...
    def _draw_pendulum_3d(self) -> None:
        ...
        self.ax.quiver(
            gravity_start[0], gravity_start[1], gravity_start[2],
            gravity_vec_world[0], gravity_vec_world[1], gravity_vec_world[2],
        ...
        self.ax.text(
            gravity_label_pos[0] + max_range * 0.1, gravity_label_pos[1], gravity_label_pos[2],
            'g↓', fontsize=16, color='#00AA00', weight='bold',
            bbox=dict(boxstyle='round,pad=0.5', facecolor='#E8F5E9', alpha=0.95, 
                     edgecolor='#00AA00', linewidth=3)
        )
        ...
        self.ax.plot(
            ...
            'g--', linewidth=2.5, alpha=0.7, label='True Vertical (World Gravity Direction)'
        )
        ...
        self.ax.plot(
            ...
            [pivot[1], elbow[1]],
            [pivot[2], elbow[2]],
            color='#2E86AB', linewidth=7, label='Upper Segment (Shoulder)', 
            alpha=0.9, zorder=5
        )
        ...
        self.ax.plot(
            ...
            [elbow[1], wrist[1]],
            [elbow[2], wrist[2]],
            color='#A23B72', linewidth=8, label='Lower Segment (Wrist)', 
            alpha=0.9, zorder=5
        )
        
        # Draw pivot point (hub) - make it prominent
        self.ax.scatter(*pivot, color='black', s=250, marker='o', 
                       label='Pivot (Hub)', edgecolors='white', linewidths=3, zorder=10)
        
        # Draw elbow joint - blue to match upper segment
        self.ax.scatter(*elbow, color='#2E86AB', s=100, marker='o', 
                       edgecolors='white', linewidths=2, zorder=9)
        
        # Draw wrist/end point (clubhead) - red to match lower segment
        self.ax.scatter(*wrist, color='#A23B72', s=180, marker='o', 
                       label='End Point (Clubhead)', edgecolors='white', linewidths=3, zorder=9)
        
        # Add text labels for segments
        # Upper segment label (midpoint)
        upper_mid = (pivot + elbow) / 2
        self.ax.text(
            upper_mid[0], upper_mid[1], upper_mid[2],
            'UPPER', fontsize=9, color='#2E86AB', weight='bold',
            bbox=dict(boxstyle='round,pad=0.2', facecolor='white', alpha=0.8, edgecolor='#2E86AB'),
            ha='center'
        )
        ...
        self.ax.text(
            lower_mid[0], lower_mid[1], lower_mid[2],
            'LOWER', fontsize=9, color='#A23B72', weight='bold',
            bbox=dict(boxstyle='round,pad=0.2', facecolor='white', alpha=0.8, edgecolor='#A23B72'),
            ha='center'
        )
        ...
        if self.dynamics.parameters.constrained_to_plane:
            ...
            self.ax.plot_surface(
                X_plane, Y_plane_rotated, Z_plane,
