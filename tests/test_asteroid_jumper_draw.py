"""Tests for asteroid_jumper.draw — sprite and primitive drawing helpers."""
...
from src.asteroid_jumper.draw import (
    STAR_POSITIONS,
    build_hud_lines,
)
...
def test_star_positions_in_unit_range():
    ...
    for fx, fy in STAR_POSITIONS:
        msg = f"fx={fx} out of [0, 1)"
        assert 0.0 <= fx < 1.0, msg
        msg = f"fy={fy} out of [0, 1)"
        assert 0.0 <= fy < 1.0, msg
