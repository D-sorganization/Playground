...
class MATLABQualityChecker:
    ...
    def check_matlab_files_exist(self) -> bool:
        ...
        if not self.matlab_dir.exists():
            logger.info(
                f"MATLAB directory not found: {self.matlab_dir} "
                "(skipping MATLAB checks)",
            )
            return False
        ...
        try:
            ...
            for i, line in enumerate(lines, 1):
                ...
                if not is_comment:
                    ...
                    if re.match(
                        r"\b(function|if|for|while|switch|try|parfor|classdef|"
                        r"arguments|properties|methods|events)\b",
                        line_stripped,
                    ):
                ...
                if re.search(r"\beval\s*\(", line_stripped):
                    issues.append(
                        f"{file_path.name} (line {i}): Avoid using eval() - potential "
                ...
                # Match both integer and floating-point literals (not part of variable
                # names)
