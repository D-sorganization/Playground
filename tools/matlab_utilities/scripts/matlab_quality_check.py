...
class MatlabQualityChecker:
    ...
    def _determine_file_type(self, lines: List[str], file_info: Dict) -> None:
        ...
        for i, line in enumerate(lines):
            ...
            func_match = re.match(r'function\s+(?:\[.*?\]\s*=\s*|\w+\s*=\s*)?(\w+)', line_clean)
            if func_match:
    ...
    def _check_documentation(self, lines: List[str], file_info: Dict) -> None:
        ...
        if file_info['type'] == 'function':
            has_input_doc = any('%' in line and ('input' in line.lower() or 'parameter' in line.lower()) 
                            for line in lines[:20])
            has_output_doc = any('%' in line and ('output' in line.lower() or 'return' in line.lower()) 
                             for line in lines[:20])
    ...
    def _check_style_issues(self, lines: List[str], file_info: Dict) -> None:
        ...
        for i, line in enumerate(lines, 1):
            ...
            if (line_clean and 
                ...
                not line_clean.endswith('...') and
                '=' in line_clean and
                not any(keyword in line_clean for keyword in ['if', 'for', 'while', 'function', 'end'])):
    ...
    def _check_best_practices(self, lines: List[str], file_info: Dict) -> None:
        ...
        for i, line in enumerate(lines, 1):
            ...
            if (i <= 5 and file_info['type'] == 'script' and 
                ('clear' in line_clean or 'clc' in line_clean)):
                file_info['issues'].append({
                    'type': 'best_practice',
                    'severity': 'warning',
    ...
    def generate_report(self, files_info: List[Dict], output_format: str = 'text') -> str:
        ...
        for severity in ['error', 'warning', 'info']:
            ...
            if issues:
                ...
                for file_path, issue in issues:
                    report_lines.append(f"  {file_path}:{issue['line']} - {issue['message']}")
                report_lines.append("")
