...
Usage:
    python eval/rollout_eval.py --policy train/outputs/imitation_policy/checkpoints/best.pth \
...
class PolicyEvaluator:
    ...
    def compute_summary_metrics(self, rollout_stats: List[Dict]) -> Dict:
        ...
        mean_speeds = [s["mean_clubhead_speed"] for s in rollout_stats]
    ...
    def generate_plots(self, rollout_stats: List[Dict]):
        ...
        ax.axvline(np.mean(max_speeds), color="red", linestyle="--", label=f"Mean: {np.mean(max_speeds):.1f} m/s")
        ...
        ax.axhline(np.mean(smoothness), color="red", linestyle="--", label=f"Mean: {np.mean(smoothness):.3f}")
...
<body>
    ...
    <table>
        ...
        <tr>
            <td>Range</td>
            <td>{summary['clubhead_speed']['max_min']:.2f} - {summary['clubhead_speed']['max_max']:.2f} m/s</td>
        </tr>
    </table>
    ...
    <table>
        ...
        <tr>
            <td>Mean Duration</td>
            <td>{summary['swing_duration']['mean']:.3f} ± {summary['swing_duration']['std']:.3f} s</td>
        </tr>
    </table>
    ...
    <table>
        ...
        <tr>
            ...
            <td>{summary['trajectory_smoothness']['mean']:.3f} ± {summary['trajectory_smoothness']['std']:.3f}</td>
        </tr>
        <tr>
